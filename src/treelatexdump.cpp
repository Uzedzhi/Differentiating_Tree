...
void LatexPrintCringe(const char *diffVarName) {
    ...
    fprintf(fp,
        ...
        "\\textbf{����� ����������.}\\\\[0.4em]\n"
        "������������� ���������� �� ������� $o\\bigl((x-\\TailorPoint)^{\\TailorAccuracy}\\bigr)$.\n"
        "\\end{coloredbox}\n\n"
...
bool HasAnyVars(Node_t *node) {
    if (node->type == TYPE_VAR)
        return true;
    
    if (node->left != NULL)
        if (HasAnyVars(node->left))
            return true;
    if (node->right != NULL)
        if (HasAnyVars(node->right))
            return true;
    return false;
}

VarPosTypes GetVarPosType(Node_t * node, const char * DiffVarName) {
    bool IsLeftVar  = HasAnyVars(node->left);
    bool IsRightVar = HasAnyVars(node->right);
...
void GetTreeGraphInTxt(DiffTree_t * tree, const char * file_name, double xfrom, double xto, double yfrom, double yto, const char *VarName) {
    ...
    for (double x = xfrom; x <= xto; x += step) {
        ...
        if (y < yfrom || y > yto || isnan(y) || fabs(y) > MAX_FUNCGRAPH_VALUE || (!isnan(prevy) && fabs(prevy - y) > MAX_FUNCGRAPH_GAP)) {
            fprintf(fp, "%lf, NAN\n", x);
        }
        else {
...
void PlaceDotInTxt(DiffTree_t *tree, double FuncRoot, const char * FileName, const char * VarName) {
    sassert(FileName, ERR_PTR_NULL);

    FILE *fp = fopen(FileName, "w");
    sassert(fp, ERR_PTR_NULL);

    fprintf(fp, "%lf %lf\n", FuncRoot, GetCalculatedAnswer(tree, tree->root, sdbm(VarName), FuncRoot));
...
char * PrintGnuplotGraphOfFunc(DiffTree_t *tree, DiffTree_t *Tailortree, DiffTree_t *Tangenttree
    ...
    GetTreeGraphInTxt(tree,         "data/data1.txt", xfrom, xto, limits.func_yfrom, limits.func_yto , VarName);
    GetTreeGraphInTxt(Tailortree,   "data/data2.txt", xfrom, xto, limits.tailor_yfrom, limits.tailor_yto, VarName);
    GetTreeGraphInTxt(Tangenttree,  "data/data3.txt", xfrom, xto, limits.tangent_yfrom, limits.tangent_yto, VarName);
    PlaceDotInTxt(tree, FuncRoot,   "data/data4.txt", VarName);
    ...
    fprintf(fp, "plot \'data/data1.txt\' with points lc rgb \"red\" title \"Func\", \\\n"
                     ...
                     "\'data/data3.txt\' with points lc rgb \"green\" title \"Tangent to Func\","
                     "\'data/data4.txt\' with points pointtype 7 pointsize 2 title \"point compos\"");
    pclose(fp);
...
TreeErr GetLimitsAndPointsFromFile(const char *file_name, double *xfrom, double *xto, ylimits * limits
    ...
    sassert(file_name,  ERR_PTR_NULL);
    sassert(xfrom,      ERR_PTR_NULL);
    sassert(xto,        ERR_PTR_NULL);
    sassert(limits,     ERR_PTR_NULL);

    FILE *fp = fopen(file_name, "r");
    sassert(fp, ERR_PTR_NULL);

    skip_line(fp);
    int scanned = fscanf(fp, "x from %lf to %lf         "
                             "y func from %lf to %lf    "
                             "y tailor from %lf to %lf  "
                             "y tangent from %lf to %lf "
                             "tailor point %lf          "
                             "tailor order %zu          "
                             "nthderivative %zu         "
                             "tangent point %lf         "
                            , xfrom, xto, &(limits->func_yfrom), &(limits->func_yto), &(limits->tailor_yfrom)
                            , &(limits->tailor_yto), &(limits->tangent_yfrom), &(limits->tangent_yto)
                            , TailorPoint, TailorAccuracy, NthDerivative, TangentPoint);
    if (scanned != 12) {
        ADD_ERROR_AND_RETURN(ERR_FILE_INVALID, "incorrect file header");
    }
...
bool CheckIfTypeWithMorePriority(Node_t *node) {
    ...
    if (left->type  == TYPE_NUM && left->value.lf < 0)
        return true;
    if (right->type == TYPE_NUM && right->value.lf < 0)
        return true;
...
#define STDOPERATION_DUMP(oper) \
    if (need_parenthises)\
        fprintf(fp, "(");\
\
    if (CheckIfTypeWithMorePriority(cur_node)) DUMPLEFT(true)\
    else DUMPLEFT(false)\
\
    fprintf(fp, " " #oper " ");\
\
    if (CheckIfTypeWithMorePriority(cur_node)) DUMPRIGHT(true)\
    else DUMPRIGHT(false)\
\
    if (need_parenthises)\
        fprintf(fp, ")");
void DumpNodesToLatexRec(Node_t * cur_node, FILE *fp, bool need_parenthises) {
    ...
    switch(cur_node->type) {
        case TYPE_NUM:
            ...
            if (!is_const) {
                if (need_parenthises) fprintf(fp, "(");
